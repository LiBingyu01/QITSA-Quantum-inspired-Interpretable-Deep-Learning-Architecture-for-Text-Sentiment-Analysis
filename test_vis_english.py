...
class projection_Euler(nn.Module):
    ...
    def forward(self, inputs):
        ...
        plt.imshow(amplitude_norm.cpu()[0,:,:].cpu().detach().numpy(), cmap='viridis', interpolation='nearest')
        plt.colorbar(label='Amplitude Information', orientation='horizontal')
        plt.title(f'Amplitude Information of Single Sentence')
        plt.ylabel(f'The Number of Word In the Sentence')
        plt.xlabel('Different Dimensions in Word Amplitude Representation')
        plt.savefig(f'{SA}_{DATA_SET}/figure2_{DATA_SET}_{SEED}/amplitude_norm_{DATA_SET}.jpg',dpi=1000)

        plt.figure(figsize=(10, 8))
        plt.imshow(phase_permute.cpu()[0,:,:].cpu().detach().numpy(), cmap='viridis', interpolation='nearest')
        plt.colorbar(label='Phase Information', orientation='horizontal')
        plt.title(f'Phase Information of Single Sentence')
        plt.ylabel(f'The Number of Word In the Sentence')
        plt.xlabel('Different Dimensions in Word Phase Representation')
        plt.savefig(f'{SA}_{DATA_SET}/figure2_{DATA_SET}_{SEED}/phase_permute_{DATA_SET}.jpg',dpi=1000)
        ...
        plt.figure(figsize=(10, 8))
        plt.imshow(real_part.cpu()[0,:,:].cpu().detach().numpy(), cmap='viridis', interpolation='nearest')
        plt.colorbar(label='Real Part', orientation='horizontal')
        plt.title('Real Part of Each Word in a Batch of Sentences')
        plt.ylabel(f'The Number of Word In the Sentence')
        plt.xlabel('Values of Each Dimension')
        plt.savefig(f'{SA}_{DATA_SET}/figure3_{DATA_SET}_{SEED}/real_part_{DATA_SET}.jpg',dpi=1000)

        imag_part = amplitude_norm * torch.sin(phase_permute)
        plt.figure(figsize=(10, 8))
        plt.imshow(imag_part.cpu()[0,:,:].cpu().detach().numpy(), cmap='viridis', interpolation='nearest')
        plt.colorbar(label='Imaginary Part', orientation='horizontal')
        plt.title('Imaginary Part of Each Word in a Batch of Sentences')
        plt.ylabel(f'The Number of Word In the Sentence')
        plt.xlabel('Values of Each Dimension')
        plt.savefig(f'{SA}_{DATA_SET}/figure3_{DATA_SET}_{SEED}/imag_part_{DATA_SET}.jpg',dpi=1000)
...
class run_complex_network(nn.Module):
    ...
    def forward(self, text, sentiment):
        ...
        if SA == 0: # 不使用 sentiment
            phase_is_sentiment = amplitude_is_WordEmbedding

        plt.figure(figsize=(10, 8))
        plt.imshow(torch.transpose(amplitude_is_WordEmbedding,0,1)[0,:,:].cpu().detach().numpy(), cmap='viridis', interpolation='nearest')
        plt.colorbar(label='WordEmbedding Vector of Text', orientation='horizontal')
        plt.title('Amplitude of Input')
        plt.ylabel('The Number of Word In the Sentence')
        plt.xlabel('Information from Different Dimensions')
        plt.savefig(f'{SA}_{DATA_SET}/figure1_{DATA_SET}_{SEED}/amplitude_is_WordEmbedding_{DATA_SET}.jpg',dpi=1000)
        
        plt.figure(figsize=(10, 8))
        plt.imshow(torch.transpose(phase_is_sentiment,0,1).cpu()[0,:,:].cpu().detach().numpy(), cmap='viridis', interpolation='nearest')
        plt.colorbar(label='Sentiment Polarity', orientation='horizontal')
        plt.title('Phase of Input')
        plt.ylabel('The Number of Word In the Sentence')
        plt.xlabel('Information from Different Dimensions')
        plt.savefig(f'{SA}_{DATA_SET}/figure1_{DATA_SET}_{SEED}/phase_is_sentiment_{DATA_SET}.jpg',dpi=1000)
        ...
        plt.figure(figsize=(10, 8))
        plt.imshow(Max_real.squeeze(1).squeeze(1).cpu(), cmap='viridis', interpolation='nearest')
        plt.title('Real Part Feature Extraction')
        plt.xlabel('Real Part Feature Extraction in Each Sentence')
        plt.ylabel('Different Classification Polarities')
        plt.savefig(f'{SA}_{DATA_SET}/figure4_{DATA_SET}_{SEED}/Max_real_{DATA_SET}.jpg',dpi=1000)

        # 6.2 -- Pooling for Imaginary Part
        Max_imag = self.MaxPool2(torch.sigmoid(Conv_imag))

        plt.figure(figsize=(10, 8))
        plt.imshow(Max_imag.squeeze(1).squeeze(1).cpu(), cmap='viridis', interpolation='nearest')
        plt.title('Imaginary Part Feature Extraction')
        plt.xlabel('Effectiveness of Imaginary Part Feature Extraction in Each Sentence')
        plt.ylabel('Different Classification Polarities')
        plt.savefig(f'{SA}_{DATA_SET}/figure4_{DATA_SET}_{SEED}/Max_imag_{DATA_SET}.jpg',dpi=1000)

        fc1 = self.fc1(torch.cat((Max_real, Max_imag), dim=3))
        plt.figure(figsize=(10, 8))
        plt.imshow(fc1.squeeze(1).squeeze(1).cpu().detach().numpy(), cmap='viridis', interpolation='nearest')
        plt.title('Final Feature Extraction Effectiveness')
        plt.xlabel('Final Feature Extraction Effectiveness in Each Sentence')
        plt.ylabel('Different Classification Polarities')
        plt.savefig(f'{SA}_{DATA_SET}/figure4_{DATA_SET}_{SEED}/fc1_{DATA_SET}.jpg',dpi=1000)

        fc2 = torch.sigmoid(self.fc2(torch.sigmoid(fc1)))

        plt.figure(figsize=(10, 8))
        plt.imshow(fc2.squeeze(1).squeeze(1).cpu(), cmap='viridis', interpolation='nearest')
        plt.title('Final Feature Extraction Effectiveness')
        plt.xlabel('Final Feature Extraction Effectiveness in Each Sentence')
        plt.ylabel('Different Classification Polarities')
        plt.savefig(f'{SA}_{DATA_SET}/figure4_{DATA_SET}_{SEED}/fc2_{DATA_SET}.jpg',dpi=1000)
...
def evaluate(model, iterator, criterion, CUDA_NUMBER):
    ...
    epoch_f1_score = 0
    epoch_recall = 0
    model.load_state_dict(torch.load(f'Best_model_of_{DATA_SET}.pt',map_location=torch.device('cpu')))
    model.eval()
    with torch.no_grad():
        for batch in iterator:
            ...
            # plt.figure(figsize=(10, 8))
            # plt.imshow(torch.transpose(text,0,1).cpu(), cmap='viridis', interpolation='nearest')
            # plt.colorbar(label='Text Sequence', orientation='horizontal')
            # plt.title('Text Sequence Information of Each Word in a Batch of Sentences')
            # plt.xlabel('"Value of Word In Sentences')
            # plt.ylabel('The Number of Sentence In the Batch')
            # plt.savefig(f'{SA}_{DATA_SET}/figure0_{DATA_SET}_{SEED}/text_{DATA_SET}.jpg',dpi=1000)

            # # plot sentiment
            # plt.figure(figsize=(10, 8))
            # plt.imshow(torch.transpose(sentiment,0,1).cpu(), cmap='viridis', interpolation='nearest')
            # plt.colorbar(label='Sentiment Polarity', orientation='horizontal')
            # plt.title('Text Sentiment Polarity Information of Each Word in a Batch of Sentences')
            # plt.xlabel('"Value of Word In Sentences')
            # plt.ylabel('The Number of Sentence In the Batch')
            # plt.savefig(f'{SA}_{DATA_SET}/figure0_{DATA_SET}_{SEED}/sentiment_{DATA_SET}.jpg',dpi=1000)
            # predictions = model(text, sentiment).squeeze(1)

            plt.figure(figsize=(10, 8))
            plt.imshow(torch.transpose(text,0,1).cpu(), cmap='viridis', interpolation='nearest')
            plt.colorbar(label='Text Sequence Number', orientation='horizontal')
            plt.title('Text Sequence Number in Each Batch')
            plt.xlabel('Text Sequence Number in Each Sentence')
            plt.ylabel('Sentence Length in Each Batch')
            plt.savefig(f'{SA}_{DATA_SET}/figure0_{DATA_SET}_{SEED}/text_{DATA_SET}.jpg',dpi=1000)

            # plot sentiment
            plt.figure(figsize=(10, 8))
            plt.imshow(torch.transpose(sentiment,0,1).cpu(), cmap='viridis', interpolation='nearest')
            plt.colorbar(label='Sentiment Polarity Score', orientation='horizontal')
            plt.title('Sentiment Polarity Score in Each Batch')
            plt.xlabel('Sentiment Polarity Score in Each Sentence')
            plt.ylabel('Sentence Length in Each Batch')
            plt.savefig(f'{SA}_{DATA_SET}/figure0_{DATA_SET}_{SEED}/sentiment_{DATA_SET}.jpg',dpi=1000)
            ...
            return 
    return epoch_loss, epoch_acc, epoch_f1_score, epoch_recall
